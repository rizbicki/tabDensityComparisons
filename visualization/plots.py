...
def _plot_sdss_raw_combined(groups, methods, output_dir):
    ...
    tex_lines = ['% Auto-generated by _plot_sdss_raw_combined -- do not edit by hand.\n']
    for metric, _, _ in METRICS_INFO:
        img_fname = f"raw_real_{metric.lower()}_sdss.png"
        cap_label = _CAPTION_LABELS.get(metric, metric)
        tex_lines.append(
            f"\\begin{{figure}}[p]\\centering\n"
            f"  \\includegraphics[width=\\linewidth]{{sdss/{img_fname}}}\n"
            f"  \\caption{{Raw {cap_label} -- SDSS, real data.}}\n"
            f"\\end{{figure}}\n"
        )
    tex_path = output_dir / "raw_real_sdss_figures.tex"
    tex_path.write_text('\n'.join(tex_lines))
